...
function Hearts({ lives, max = 3 }: { lives: number; max?: number }) {
  return <div className="flex gap-0.5">{Array.from({ length: max }).map((_, i) => <Heart key={i} className={`w-4 h-4 ${i < lives ? 'text-red-500 fill-red-500' : 'text-gray-300 dark:text-gray-600'}`} />)}</div>
}
function Avatar({ player, size = 'md' }: { player: PlayerInfo; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return player.profile_picture
    ...
    : <div className={`${sz} rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold ring-2 ring-white dark:ring-gray-800`}>{player.username[0].toUpperCase()}</div>
}
...
function EffectBadges({ effects }: { effects: Record<string, number> }) {
  ...
  return (
    <div className="flex gap-0.5 flex-wrap mt-0.5">
      {items.map(([id]) => (
        <span key={id} title={id} className={`text-[10px] px-1 rounded-full ${BUFF_IDS.has(id) ? 'bg-emerald-100 dark:bg-emerald-900/60' : 'bg-rose-100 dark:bg-rose-900/60'}`}>
          {EFFECT_EMOJIS[id] ?? '?'}
        </span>
      ))}
    </div>
...
function InventoryPanel({ inventory, players, currentUserId, sendMessage }: {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow border border-indigo-100 dark:border-indigo-900/40 p-3">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-indigo-500 mb-2">🎒 Your Items</p>
            ...
            <motion.div key={id} className="relative">
              <motion.button
                ...
                className={`flex items-center gap-1 px-2 py-1.5 rounded-xl text-xs font-bold border-2 transition-all ${activeItem === id
                  ? 'border-indigo-500 ring-2 ring-indigo-300 bg-indigo-50 dark:bg-indigo-950/40'
                  : isBuff
...
function BuffEventBanner({ event, currentUserId }: { event: BuffEvent; currentUserId: number }) {
  ...
  const bg = event.isBuff ? 'from-emerald-500 to-teal-600' : 'from-rose-500 to-purple-700'
  return (
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-4 sm:p-5">
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-4">
          ...
          { value: 'team_vs_team', label: 'Team vs Team', desc: 'Compete as a team', icon: Users, color: 'rose' }] as const
            ...
            <button key={value} onClick={() => setMode(value)} className={`p-3 sm:p-4 rounded-xl border-2 text-left transition-all ${mode === value ? color === 'indigo' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/30' : 'border-rose-500 bg-rose-50 dark:bg-rose-950/30' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
              <Icon className={`w-4 h-4 sm:w-5 sm:h-5 mb-1.5 ${mode === value ? color === 'indigo' ? 'text-indigo-600' : 'text-rose-500' : 'text-gray-400'}`} />
        ...
        <button onClick={handleCreate} disabled={creating} className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-all shadow-md shadow-indigo-500/25 text-sm sm:text-base">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-4 sm:p-5">
        ...
        <div className="flex gap-2">
          <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} maxLength={6} placeholder="ABC123"
            className="flex-1 px-4 py-2.5 border-2 border-gray-200 dark:border-gray-700 rounded-xl text-center text-xl font-mono font-bold tracking-widest bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:border-indigo-400 uppercase" />
...
function LobbyView({ gameState, currentUserId, sendMessage, onLeave }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-3 sm:space-y-5">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-3 sm:p-5">
        <div className="flex items-start sm:items-center justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-3xl sm:text-4xl font-black tracking-widest text-indigo-600 dark:text-indigo-400 font-mono">{gameState.roomCode}</p>
          </div>
          <div className="flex gap-2 items-center flex-wrap">
            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${gameState.mode === 'battle_royale' ? 'bg-indigo-100 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400' : 'bg-rose-100 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400'}`}>
          ...
          {gameState.players.map(p => (
            <div key={p.user_id} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl bg-gray-50 dark:bg-gray-800">
              ...
              {isHost && p.user_id !== currentUserId && (
                <button onClick={() => kick(p.user_id)} className="p-1 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 text-gray-400 hover:text-red-500 transition-all"><X className="w-3.5 h-3.5" /></button>
              )}
            </div>
          ))}
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-3 sm:p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">{[1, 2, 3, 4, 5, 6].map(l => <button key={l} onClick={() => setHskLevel(l)} className={`w-9 h-9 rounded-xl text-sm font-semibold transition-all ${hskLevel === l ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}>{l}</button>)}</div>
            </div>
            <div>
      ...
      {gameState.error && <p className="text-center text-sm text-red-500">{gameState.error}</p>}
    </motion.div>
  )
}
    ...
    <div className="flex items-center justify-center min-h-[60vh]">
      <AnimatePresence mode="wait">
        <motion.div key={seconds} initial={{ scale: 0.4, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 1.6, opacity: 0 }} transition={{ duration: 0.4 }} className="text-center">
          <p className="text-8xl sm:text-9xl font-black text-indigo-600 dark:text-indigo-400">{seconds}</p>
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto space-y-3 sm:space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-100 dark:border-gray-800 p-3 sm:p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className={`flex items-center gap-1 font-bold text-base sm:text-lg ${timeLeft <= 5 ? 'text-red-500' : 'text-gray-800 dark:text-gray-200'}`}>
        ...
        <div className="h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
          <motion.div className={`h-full rounded-full ${timeLeft <= 5 ? 'bg-red-500' : myFx['time_bonus'] ? 'bg-emerald-500' : myFx['time_cut'] ? 'bg-orange-500' : 'bg-indigo-500'}`} animate={{ width: `${timerPct}%` }} transition={{ duration: 0.25 }} />
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
        {gameState.players.map(p => (
          <div key={p.user_id} className={`flex items-center gap-1 px-1.5 py-1 rounded-xl border ${p.eliminated ? 'border-red-200 bg-red-50 dark:bg-red-950/20 opacity-50' : p.user_id === currentUserId ? 'border-indigo-300 bg-indigo-50 dark:bg-indigo-950/30' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800'}`}>
            <Avatar player={p} size="sm" />
            <div className="hidden sm:block ml-0.5">
              ...
              {gameState.mode === 'battle_royale' ? <Hearts lives={p.lives} max={maxLives} /> : <p className="text-[10px] text-indigo-600 font-bold">{p.score}p</p>}
              <EffectBadges effects={p.active_effects ?? {}} />
            </div>
            {gameState.answeredUserIds.includes(p.user_id) && !p.eliminated && <Check className="w-3 h-3 text-green-500 ml-1" />}
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-3 sm:p-5">
        ...
        {me?.eliminated && <p className="text-center text-sm text-red-500 mt-3 font-semibold">☠️ You've been eliminated — watching as spectator</p>}
      </div>
    </motion.div>
  )
}
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-2xl mx-auto space-y-3 sm:space-y-4">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-emerald-300 dark:border-emerald-700 p-4 sm:p-5 text-center">
        ...
        <p className={`font-bold text-emerald-700 dark:text-emerald-400 ${q.question_type === 'multiple_choice' ? 'text-3xl sm:text-4xl font-chinese' : q.question_type === 'pinyin_match' ? 'text-2xl sm:text-3xl tracking-wide' : 'text-xl sm:text-2xl'}`}>{reveal.correctText}</p>
        ...
        {q.question_type !== 'pinyin_match' && <p className="text-indigo-500 mt-1 text-sm">{q.pinyin}</p>}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-3 sm:p-4">
        ...
        <div className="space-y-1.5">
          {reveal.players.map(p => (
            <div key={p.user_id} className={`flex items-center gap-2 sm:gap-3 p-2 rounded-xl ${p.result === 'correct' ? 'bg-emerald-50 dark:bg-emerald-950/20' : p.result === 'wrong' ? 'bg-red-50 dark:bg-red-950/20' : 'bg-gray-50 dark:bg-gray-800 opacity-50'}`}>
              <Avatar player={p} size="sm" />
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border-2 border-yellow-300 dark:border-yellow-700 p-4 sm:p-6 text-center">
        ...
        {isBR && over.winner ? (
          <>
            ...
            <p className="text-indigo-600 dark:text-indigo-400 font-semibold">{over.winner.score} points</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-4 py-6 sm:py-8 pb-16">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 mb-6 sm:mb-8">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-md shadow-indigo-500/30"><Swords className="w-5 h-5 text-white" /></div>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100">Battle Arena</h1>
          {roomCode && (
            <p className="text-sm text-gray-500 dark:text-gray-400 flex items-center gap-1.5">
              <span className={`w-2 h-2 rounded-full ${connectionStatus === 'connected' ? 'bg-green-500' : connectionStatus === 'failed' ? 'bg-red-500' : 'bg-yellow-400 animate-pulse'}`} />
